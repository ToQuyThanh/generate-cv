...
const schema = z.object({
  ...
  // Backend binding:"required,min=8" — khớp với min=8 ở đây
  password: z
    .string()
...
export default function RegisterPage() {
  ...
  const onSubmit = async ({ full_name, email, password }: FormData) => {
    try {
      ...
      // Fetch subscription riêng (user mới sẽ có free plan)
      try {
        const sub = await userApi.getSubscription()
        setSubscription(sub)
      } catch {
        // Không block register nếu fetch sub lỗi
      }
        ...
        <div className="w-full max-w-sm space-y-6 animate-fade-in">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
            ...
            <p className="text-xs text-center text-muted-foreground">
              ...
              <Link href="/terms" className="underline hover:text-primary">Điều khoản</Link> và{' '}
