...
const mockCVs: CVListItem[] = [
  { id: 'cv-1', title: 'CV Frontend Dev', template_id: 'template_modern_01', color_theme: '#1a56db', created_at: '2026-04-01T00:00:00Z', updated_at: '2026-04-10T00:00:00Z' },
  { id: 'cv-2', title: 'CV Backend Dev', template_id: 'template_classic_01', color_theme: '#0f9f6e', created_at: '2026-03-20T00:00:00Z', updated_at: '2026-04-05T00:00:00Z' },
]
...
describe('DashboardPage', () => {
  ...
  it('hiển thị danh sách CV sau khi load', async () => {
    vi.mocked(cvApi.list).mockResolvedValue({ data: mockCVs, total: 2, page: 1, limit: 12, total_pages: 1 })
  ...
  it('hiển thị empty state khi không có CV', async () => {
    vi.mocked(cvApi.list).mockResolvedValue({ data: [], total: 0, page: 1, limit: 12, total_pages: 0 })
  ...
  it('hiển thị banner nâng cấp cho free user có CV', async () => {
    vi.mocked(cvApi.list).mockResolvedValue({ data: mockCVs, total: 2, page: 1, limit: 12, total_pages: 1 })

    render(<DashboardPage />)

    await waitFor(() => {
      expect(screen.getByText(/Nâng cấp để dùng AI/)).toBeInTheDocument()
    })
  })

  it('tạo CV mới và redirect vào editor', async () => {
    vi.mocked(cvApi.list).mockResolvedValue({ data: [], total: 0, page: 1, limit: 12, total_pages: 0 })
    vi.mocked(cvApi.create).mockResolvedValue({ id: 'new-cv', title: 'CV của tôi', template_id: 'template_modern_01', color_theme: '#1a56db', sections: [], created_at: '', updated_at: '' })
    ...
    await waitFor(() => {
      expect(cvApi.create).toHaveBeenCalled()
      expect(mockPush).toHaveBeenCalledWith('/cv/new-cv')
    })
  })
})
